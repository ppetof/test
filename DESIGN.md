...
`Hero_video2.mp4` (8s, 1664×1248) scrubbed by scroll: it opens tight on a
rider forcing the bike up a rock face and pulls back to the whole forest.
The visitor scrolls, the rider climbs, the world opens.
...
| .60 → .80 | last 2s of film: blur ramps 0 → 16px, veil ramps in |
...
6. **The blur is dropped once the veil covers it** (`.is-covered` past .93),
   so nothing pays for a filter nobody can see.
7. **Blur is the expensive part of this hero, and only during .60 → .93.**
   Measured in software rendering: p95 16.7ms with 0 long frames outside the
   blur phase, 33.4ms with ~10 inside it. Rendering the video at half size
   and scaling it back up was tried — the geometry worked, the raster really
   was quartered, and it changed nothing measurable, so it was reverted
   rather than kept as unproven complexity. On GPU hardware the picture may
   differ; re-measure before reaching for it again.
